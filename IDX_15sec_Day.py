      ...
      if (condIDX and os.path.getsize(strDSTIDX1) <=2048):
          ...
          if condIDX:             
            ...
            if Cond1:
              ...
              u1=open(strDSTIDX1)
              rows = u1.read().split('\n')
              outfileT1=outAB+"Day_"+YYYY+MM+DD+'.csv'
              f1 = open(outfileT1, 'w')
              ...
              for row in rows:
                ...
                if (sectionstart==1 and (row!='')):
                  if ('時間,發行量加權股價指數' in row):
                    f1.write('指數代號,日期,開盤價,最高價,最低價,收盤價,指數名稱\n')    
                  else:	   
                    if RowCount>=3:  
                      item=row.split(',')
                      ...
                      if RowCount==3:        
                        ...
                        for i in range(0,34):
                          ...
                          if IdxValue < TickerLow[i]:
                             TickerLow[i]= IdxValue  #Low
              for i in range(0,34):            
                f1.write(Ticker[i]+','+YYYY+'/'+MM+'/'+DD+','+str(TickerOpen[i])+','+str(TickerHigh[i])+','+str(TickerLow[i])+','+str(TickerClose[i])+','+TickerInfo[i]+'\n')                              
              f1.close()  
              u1.close()
